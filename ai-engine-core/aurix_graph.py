...
def logic_agent_node(state: AurixState):
    """
    Triage node: Now uses pre-computed triage data from the Hyper-Batch step.
    If _triage_reasoning exists, we skip the API call entirely.
    Falls back to API call if the finding was injected without pre-triage.
    """
    vuln = state['current_vuln']
    
    # If the worker already triaged this finding, use that result directly
    if vuln.get('_triage_reasoning'):
        return {
            "confidence_score": 1.0,  # Already confirmed exploitable by the batch triage
            "is_exploitable": True,
            "logic_reasoning": vuln['_triage_reasoning']
        }
    
    # Fallback: If somehow a finding arrives without pre-triage, do it live
